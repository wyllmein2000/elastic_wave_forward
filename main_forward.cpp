...
int main (int argc, char **argv) {
   ...
   PARA.GetElasVel (vp, vs, rho);
   PARA.GetHeader (src, myid);
   PARA.MapVpToLam (lam, mu, rho, vp, vs);
   MinMaxVel (&par.vmin, &par.vmax, lam, mu, rho, geo.nx * geo.nz);

   // *** modeling
   for (int is = iso + myid; is < ns; is += nprocs) {

        cout << " |||| myid=" << myid << " is=" << is+1 << endl;

        stringstream sidx;
        sidx << setfill('0') << setw(4) << is + 1;
        string fo("shot1/etr_");
        string fo1("shot1/etr_");
        string fo2("shot1/etr_");
        fo = fo + sidx.str() + ".bin";
        ...
        forward (uo, lam, mu, rho, src[is], par, geo, iw, is, myid);
        writedata((char *)fo.c_str(), uo, m);

        if (par.iway == 1) {
            if (myid == 0) cout << " *** Only direct waves ... \n";
            MakeDirect (ud, lam, mu, rho, src[is], par, geo, 0, is, myid);
            asub(uo, uo, ud, m);
...
void forward (float *uo, float *lam, float *mu, float *rho, Shot src, Fwdpar par, Geometry geo, int iw, int is, int myid) {
        ...
        int isrc = par.isrc;      // source type
        int izf = -1;             // free surface
        ...
        float *lam1 = new float[nxg * nzg];
        float *mu1 = new float[nxg * nzg];

        interp2d (rho1, fxl, fzl, dx, dz, nxg, nzg, rho, geo.bx, 0.0, geo.dx, geo.dz, geo.nx, geo.nz); 
        interp2d (lam1, fxl, fzl, dx, dz, nxg, nzg, lam, geo.bx, 0.0, geo.dx, geo.dz, geo.nx, geo.nz); 
        interp2d ( mu1, fxl, fzl, dx, dz, nxg, nzg,  mu, geo.bx, 0.0, geo.dx, geo.dz, geo.nx, geo.nz); 

        // FD simulation
        fd2d fdshot (nxg, nzg, ntc, dx, dz, dt, par.nxpml, par.nzpml, par.nxor);
        fdshot.snapset (dxo, 0, dzo, 10*dt, nxo, 1, nzo, nto);
        fdshot.initabc (izf, par.fp, lam1, mu1, rho1);
        fdshot.forward (uw, lam1, mu1, rho1, izs, izg, isrc, iw);

        interp2d ( uo, 0.0, 0.0, dxo, dto, nxo, nto, uw, fxl, 0.0, dx, dt, nxg, ntc);
...
void MakeDirect (float *ud, float *lam, float *mu, float *rho, Shot src, Fwdpar par, Geometry geo, int iw, int is, int myid) {
    int m, m0, nv = geo.nx * geo.nz;
    float *rho1 = new float[nv];
    float *lam1 = new float[nv];
    float *mu1 = new float[nv];

    memcpy (rho1, rho, nv * sizeof(float));
    memcpy (lam1, lam, nv * sizeof(float));
    memcpy ( mu1,  mu, nv * sizeof(float));
    ...
    forward (ud, lam1, mu1, rho1, src, par, geo, iw, is, myid);
...
void MinMaxVel (float *vmin, float *vmax, float *lam, float *mu, float *rho, int n) {
  float p, q; 
  float v1 = sqrt((lam[0] + 2.0 * mu[0]) / rho[0]);
  float v2 = sqrt(mu[0] / rho[0]);

  for (int i = 1; i < n; i ++) {
       p = sqrt((lam[i] + 2.0 * mu[i]) / rho[i]);
       q = sqrt(mu[i] / rho[i]);
       if (v1 < p) v1 = p;
       if (v2 > q) v2 = q;
  }
